...
function FloatingInput({ id, label, type: initialType, error, register, name, delay = 0 }: FloatingInputProps) {
  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    try {
      ...
      if (result.success && result.user) {
        setIsSuccess(true);
        const isTherapist = result.user.isTherapist;
        setTimeout(() => {
          onSuccess();
          if (isTherapist) {
      ...
      <div style={{ width: '100%', maxWidth: '380px' }}>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <motion.div
            ...
            style={{ textAlign: 'right', marginBottom: '24px', marginTop: '-12px' }}
          >
            <Link
